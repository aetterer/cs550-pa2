...
int main(int argc, char **argv) {
    ...
    listener_socket = init_server(port);
...
void * handle_connection(void *pclient) {
    ...
    while (1) {
        ...
        if (strncmp(cmd, "list", len) == 0) {
            ...
            //    fclose(lp);
            //}
            printf("Received `%s' command from %s\n", cmd, uid);
            ...
            //    fclose(lp);
            //}
            printf("Sent list of %d filenames to %s\n", num_files, uid);
        }
        ...
        if (strncmp(cmd, "download", len) == 0) {
            ...
            //    fclose(lp);
            //}
            printf("Received `%s  %s' command from %s\n", cmd, filename, uid);

            // Get the file from the hash table and chose a host.
            struct file_info *f = get_file_ht_entry(filename);
            //char chosen_uid[UID_LEN] = "\0";
            //while (strncmp(chosen_uid, "\0", UID_LEN) == 0) {
            //    int r = rand() % MAX_PEER_NODES;
            //    strncpy(chosen_uid, f->uids[r], UID_LEN);
            //    printf("FOUND UID %s\n", chosen_uid);
            //    break;
            //}
            //struct client_info *c = get_client_ht_entry(chosen_uid);
            ...
            //    fclose(lp);
            //}
            printf("Sent host ip and port `%s:%s' to %s\n",c->ip, c->sport, uid);
            
            // Now receive the status of the file.
            recv_stat(client_socket, &stat);

            // Add the file to the client's list of files.
            c->num_files = c->num_files + 1;
            c->files = realloc(c->files, c->num_files * MAX_FN_LEN);
            strncpy(c->files[c->num_files - 1], filename, MAX_FN_LEN);

            // Add the client as a host of the file.
            for (int i = 0; i < MAX_PEER_NODES; i++) {
                if (strncmp(f->uids[i], "\0", UID_LEN) == 0) {
                    strncpy(f->uids[i], uid, UID_LEN);
                }
            }
...
void unregister_client(char *uid) {
    // Remove client form HT.
    struct client_info *c = client_ht_delete(uid);

    // Loop through each file in the entry and remove the client from the list
    // of hosts for that file.
    int num_files = c->num_files;
    for (int i = 0; i < num_files; i++) {
        char *filename = c->files[i];
        struct file_info *f = get_file_ht_entry(filename);
        int num_hosts = f->num_hosts;
        for (int j = 0; j < num_hosts; j++) {
            if (strncmp(f->uids[i], c->uid, UID_LEN) == 0) {
                strncpy(f->uids[i], "\0", UID_LEN);
                break;
            }
        }
        f->num_hosts = f->num_hosts - 1;
        // If no more nodes host the file, then remove it from the HT.
        if (f->num_hosts == 0) {
            f = file_ht_delete(filename);
            free(f);
        }
    }
